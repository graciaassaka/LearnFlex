...
class UploadSectionUseCaseTest {
    ...
    fun `upload should return success when succeeds`() = runTest {
        ...
        coEvery { repository.insert(path, section) } returns Result.success(Unit)

        val result = useCase(path, section)

        coVerify(exactly = 1) { repository.insert(path, section) }
    ...
    fun `upload should return failure when fails`() = runTest {
        ...
        coEvery { repository.insert(path, section) } returns Result.failure(exception)
