...
class RetrieveCurriculaByStatusUseCaseTest {
    private lateinit var retrieveCurriculaByStatusUseCase: RetrieveCurriculaByStatusUseCase
    private lateinit var repository: CurriculumRepository

    @Before
    fun setUp() {
        repository = mockk()
        retrieveCurriculaByStatusUseCase = RetrieveCurriculaByStatusUseCase(repository)
    }
    ...
    fun `invoke should return success with filtered curricula list`() = runTest {
        ...
        // Act
        val result = retrieveCurriculaByStatusUseCase(status)
