...
class UpdateCurriculumUseCaseTest {
    ...
    fun `invoke should return success when update succeeds`() = runTest {
        ...
        coEvery { repository.update(path, curriculum) } returns Result.success(Unit)

        // Act
        val result = updateCurriculumUseCase(path, curriculum)

        // Assert
        coVerify(exactly = 1) { repository.update(path, curriculum) }
    ...
    fun `invoke should return failure when update fails`() = runTest {
        ...
        coEvery { repository.update(path, curriculum) } returns Result.failure(exception)
