...
class UpdateModuleUseCaseTest {
    ...
    fun `update should return success when succeeds`() = runTest {
        ...
        coEvery { repository.update(path, module) } returns Result.success(Unit)

        val result = useCase(path, module)

        coVerify(exactly = 1) { repository.update(path, module) }
    ...
    fun `update should return failure when fails`() = runTest {
        ...
        coEvery { repository.update(path, module) } returns Result.failure(exception)
