...
class UploadModuleUseCaseTest {
    ...
    fun `upload should return success when succeeds`() = runTest {
        ...
        coEvery { repository.insert(path, module) } returns Result.success(Unit)

        val result = useCase(path, module)

        coVerify(exactly = 1) { repository.insert(path, module) }
    ...
    fun `upload should return failure when fails`() = runTest {
        ...
        coEvery { repository.insert(path, module) } returns Result.failure(exception)
