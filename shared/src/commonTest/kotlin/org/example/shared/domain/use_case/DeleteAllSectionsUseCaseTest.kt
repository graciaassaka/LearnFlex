...
class DeleteAllSectionsUseCaseTest {
    ...
    fun `deleteAll should return success when succeeds`() = runTest {
        ...
        coEvery { repository.deleteAll(path, sections) } returns Result.success(Unit)

        val result = useCase(path, sections)

        coVerify(exactly = 1) { repository.deleteAll(path, sections) }
    ...
    fun `deleteAll should return failure when fails`() = runTest {
        ...
        coEvery { repository.deleteAll(path, sections) } returns Result.failure(exception)
