...
class RetrieveLessonsByMinQuizScoreUseCaseTest {
    private lateinit var retrieveLessonsByMinQuizScoreUseCase: RetrieveLessonsByMinQuizScoreUseCase
    private lateinit var repository: LessonRepository

    @Before
    fun setUp() {
        repository = mockk()
        retrieveLessonsByMinQuizScoreUseCase = RetrieveLessonsByMinQuizScoreUseCase(repository)
    }
    ...
    fun `invoke should return lesson ids flow when get succeeds`() = runTest {
        ...
        val result = retrieveLessonsByMinQuizScoreUseCase(path, minScore)
