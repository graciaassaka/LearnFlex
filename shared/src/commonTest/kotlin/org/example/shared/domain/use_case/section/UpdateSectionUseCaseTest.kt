...
class UpdateSectionUseCaseTest {
    ...
    fun `update should return success when succeeds`() = runTest {
        ...
        coEvery { repository.update(path, section, any()) } returns Result.success(Unit)

        val result = useCase(path, section)

        coVerify(exactly = 1) { repository.update(path, section, any()) }
    ...
    fun `update should return failure when fails`() = runTest {
        ...
        coEvery { repository.update(path, section, any()) } returns Result.failure(exception)
