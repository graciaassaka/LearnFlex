...
class UploadLessonUseCaseTest {
    ...
    fun `invoke should return success when upload succeeds`() = runTest {
        ...
        coEvery { repository.insert(path, lesson) } returns Result.success(Unit)

        // Act
        val result = uploadLessonUseCase(path, lesson)

        // Assert
        coVerify(exactly = 1) { repository.insert(path, lesson) }
    ...
    fun `invoke should return failure when upload fails`() = runTest {
        ...
        coEvery { repository.insert(path, lesson) } returns Result.failure(exception)
