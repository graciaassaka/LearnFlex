...
class GetModuleUseCase(private val repository: ModuleRepository) {
     ...
     * @throws IllegalArgumentException If the path does not end with [DataCollection.MODULES].
     */
    operator fun invoke(path: String, id: String) = flow {
        require(path.split("/").last() == DataCollection.MODULES.value) {
            "The path must end with ${DataCollection.MODULES.value}"
        }
